...
int main() {
    ...
    h_C_gpu = (float*)malloc(size_C); // used later to cpy back to host.
    ...
    for (int i = 0; i < 20; i++) {
        double start_time = get_time();
        matmul_gpu<<<gridDim, blockDim>>>(d_A, d_B, d_C, M, K, N);
        cudaDeviceSynchronize(); // forces the host to wait until all previously issued GPU operations are completed
        double end_time = get_time();
